        ...
        if  0.0150 < mevent.lspdist2d() < 2 :   # apply fiducial cuts
            ...
            for vtx_ntuple1 in vertices_from_ntuple1 :
                 ...
                 if math.fabs(ROOT.reco.deltaPhi(mevent.gen_lsp_phi[0],vtx_ntuple1_phi)) < 1.57:
                    if exclude_beampipe and !jmt.Geometry.inside_beampipe(is_mc, vtx_ntuple1.x, vtx_ntuple1.y): 
                       if vtx_ntuple1.ntracks()>=5 and mag(vtx_ntuple1.x - mevent.bsx_at_z(vtx_ntuple1.z), vtx_ntuple1.y - mevent.bsy_at_z(vtx_ntuple1.z)) > 0.0100 and vtx_ntuple1.rescale_bs2derr < 0.0025 :
                            nsv_ntuple1 += 1
            ...
            for vtx_ntuple2 in vertices_from_ntuple2 :
                 ...
                 if math.fabs(ROOT.reco.deltaPhi(mevent.gen_lsp_phi[0],vtx_ntuple2_phi)) < 1.57:
                    if (exclude_beampipe) and (!jmt.Geometry.inside_beampipe(is_mc, vtx_ntuple2.x, vtx_ntuple2.y)) and (vtx_ntuple2.ntracks()>=5) and (mag(vtx_ntuple2.x - mevent.bsx_at_z(vtx_ntuple2.z), vtx_ntuple2.y - mevent.bsy_at_z(vtx_ntuple2.z)) > 0.0100) and (vtx_ntuple2rescale_bs2derr < 0.0025):
                        nsv_ntuple2 += 1
