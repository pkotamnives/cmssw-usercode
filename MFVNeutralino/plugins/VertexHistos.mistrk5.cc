...
private:
	...
	TH2F* h_2D_sv_tracks_no_less_avg_pt_sig_shared_tracks_nshj1_large_nsv2;
	...
	do_scatterplots(cfg.getParameter<bool>("do_scatterplots"))
{
	...
	h_2D_sv_tracks_no_less_ratio_sum_pt_shared_tracks_nshj1_large_nsv2 = fs->make<TH2F>("h_2D_sv_tracks_no_less_ratio_sum_pt_shared_tracks_nshj1_large_nsv2", "nsv = 2, absdPhi01 > 0.5, nshj=1, no minor's_{ratio sum pt} shared tracks ; # major tracks; # minor tracks", 50, 0, 50, 50, 0, 50);
	h_2D_sv_tracks_no_less_ratio_avg_pt_shared_tracks_nshj1_large_nsv2 = fs->make<TH2F>("h_2D_sv_tracks_no_less_ratio_avg_pt_shared_tracks_nshj1_large_nsv2", "nsv = 2, absdPhi01 > 0.5, nshj=1, no minor's_{ratio avg pt} shared tracks ; # major tracks; # minor tracks", 50, 0, 50, 50, 0, 50);
	h_2D_sv_tracks_no_less_avg_pt_shared_tracks_nshj1_large_nsv2 = fs->make<TH2F>("h_2D_sv_tracks_no_less_avg_pt_shared_tracks_nshj1_large_nsv2", "nsv = 2, absdPhi01 > 0.5, nshj=1, no minor's_{avg pt} shared tracks ; # major tracks; # minor tracks", 50, 0, 50, 50, 0, 50);
	h_2D_sv_tracks_no_less_avg_pt_sig_shared_tracks_nshj1_large_nsv2 = fs->make<TH2F>("h_2D_sv_tracks_no_less_avg_sig_pt_shared_tracks_nshj1_large_nsv2", "nsv = 2, absdPhi01 > 0.5, nshj=1, no minor's_{avg sig pt} shared tracks ; # major tracks; # minor tracks", 50, 0, 50, 50, 0, 50);

	//start to apply the >= n ratio cut

	h_less_dphi_more_shared_tracks_large_nsv2_all_fig2 = fs->make<TH1F>("h_less_dphi_more_shared_tracks_large_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; min(dphi(SV,a shared jet)) most shared tracks ?", 2, 0, 2);
	h_ratio_nsharedjets_large_nsv2_all_fig2 = fs->make<TH1F>("h_ratio_nsharedjets_large_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4;ratios of shared jets (>=1);arb. units", 50, 0, 10);
	h_more_ratio_sum_major_sv_nsv2_all_fig2 = fs->make<TH1F>("h_more_ratio_sum_major_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; more sum p_{T} ratio(shared/SV's tracks) is major SV ?", 2, 0, 2);
	h_more_ratio_avg_major_sv_nsv2_all_fig2 = fs->make<TH1F>("h_more_ratio_avg_major_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; more avg p_{T} ratio(shared/SV's tracks) is major SV ?", 2, 0, 2);
	h_more_pT_avg_major_sv_nsv2_all_fig2 = fs->make<TH1F>("h_more_pT_avg_major_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; more avg p_{T} shared tracks is major SV ?", 2, 0, 2);
	h_more_pT_sig_avg_major_sv_nsv2_all_fig2 = fs->make<TH1F>("h_more_pT_sig_avg_major_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; more avg p_{T} sig shared tracks is major SV ?", 2, 0, 2);
	
	h_diff_pT_avg_major_minor_sv_nsv2_all_fig2 = fs->make<TH1F>("h_diff_pT_avg_major_minor_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; major4 shared avg p_{T} - minor4 shared avg p_{T}", 20, -100, 100);
	h_diff_ratio_pT_avg_major_minor_sv_nsv2_all_fig2 = fs->make<TH1F>("h_diff_ratio_pT_avg_major_minor_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; major4 ratio - minor4 ratio (shared avg p_{T}/SV all avg p_{T})", 40, -2, 2);
	h_ratio_diff_pT_sum_major_minor_sv_nsv2_all_fig2 = fs->make<TH1F>("h_ratio_diff_pT_sum_major_minor_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; \frac{major4 shared sum pT - minor4 shared sum pT}{major4 shared sum pT + minor4 shared sum pT}", 40, -2, 2);
	h_ratio_diff_pT_avg_major_minor_sv_nsv2_all_fig2 = fs->make<TH1F>("h_ratio_diff_pT_avg_major_minor_sv_nsv2_all_fig2", "nsv = 2, absdeltaphi01 > 0.5, shared ratios >= 4; \frac{major4 shared avg pT - minor4 shared avg pT}{major4 shared avg pT + minor4 shared avg pT}", 40, -2, 2);
				...
				if (fabs(reco::deltaPhi(phi0, phi1)) > 0.5) {	//start no split vertex 
					...
					if (nsharedjets == 1) {
						...
						if ((sum_pt_sig_shared_sv0 / (sv0_sharedjet_1_which_trk_idx.size())) >= (sum_pt_sig_shared_sv1 / (sv1_sharedjet_1_which_trk_idx.size()))) {		 // sv0 is major vtx	sv1 got removed if equal 
							...
							if ((sv0_track_which_idx.size() >= 5) && (sv1_track_which_idx_no_shared_track.size() >= 5)) {
								if (sv0_track_which_idx.size() >= sv1_track_which_idx_no_shared_track.size()) {
									h_2D_sv_tracks_no_less_avg_pt_sig_shared_tracks_nshj1_large_nsv2->Fill(sv0_track_which_idx.size(), sv1_track_which_idx_no_shared_track.size());
								}
								else {
